...
cartRouter.post('/api/addCartProduct',async(req,res)=>{
    try{
        ...
        console.log(product_id,quantity_product)
        let isExist = await CartModel.findOne({product_id:product_id});
        console.log(isExist);
        if(isExist){
            let num = isExist.quantity_product;
            isExist =  await CartModel.findOneAndUpdate({product_id:product_id},{"quantity_product":num+quantity_product},{new:true});
        console.log(isExist);
