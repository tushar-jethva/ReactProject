...
const ProductDetails = () => {
  ...
  const addCart = async () => {
    setCartLoading(true);
    const token = localStorage.getItem("krist-app-token");
    const response =  await addToCart(token, { product_id: productt?._id, quantity_product: 1 });
  ...
  const discountedPrice =  product.price -  (product.price * product.discount) / 100.00;
  ...
  return (
    <Container>
      {loading ? (
        ...
        <Wrapper>
          <ImageWrapper>
            <Image src={productt?.images[0]} />
          </ImageWrapper>
          <Details>
            <div>
              <Title>{productt?.name}</Title>
              <Name>{productt?.name}</Name>
            </div>
            <Rating value={3.5} />
            <Price>
          ...
          <Percent>${product?.discount  }% Off</Percent>
            ...
            <Desc>{productt?.description}</Desc>
            <ButtonWrapper>
              <Button
                text="Add to Cart"
                full
